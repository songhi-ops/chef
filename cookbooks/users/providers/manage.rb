...
action :remove do
  if Chef::Config[:solo] and not chef_solo_search_installed?
    Chef::Log.warn("This recipe uses search. Chef Solo does not support search unless you install the chef-solo-search cookbook.")
      ...
      search(new_resource.data_bag, "groups#{node.chef_environment}:#{new_resource.search_group} AND action:remove") do |rm_user|
...
action :create do
  ...
  if Chef::Config[:solo] and not chef_solo_search_installed?
    Chef::Log.warn("This recipe uses search. Chef Solo does not support search unless you install the chef-solo-search cookbook.")
      ...
      search(new_resource.data_bag, "groups#{node.chef_environment}:#{new_resource.search_group} AND NOT action:remove") do |u|
      u['username'] ||= u['id']
