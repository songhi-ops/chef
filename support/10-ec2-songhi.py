...
def aws_launch (template, name, instance_type=None, region=None):
    ...
    r = template['connection'].run_instances(image_id=template['image_id'], key_name=template['key_name'], instance_type=instance_size, placement=region, subnet_id=template['subnet_id'], disable_api_termination=True, security_group_ids=template['security_group_ids'])
    ...
    user_data = "ssh -o 'ProxyCommand ssh -i /Volumes/untitled/SongHi/operations.pem root@54.86.212.19 nc " + instance.private_ip_address + " 22' -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i /Volumes/untitled/SongHi/operations.pem root@" + instance.private_ip_address + " 'curl --connect-timeout 10 -u operations:smothcooperat0r http://10.0.0.74:801/hosts.php?hostname=" + name + " | bash '" 
    print user_data
    os.system(user_data)
